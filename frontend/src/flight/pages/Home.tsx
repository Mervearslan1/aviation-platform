...
export function Home() {
  ...
  return (
    <section className="grid gap-8 lg:grid-cols-[1.2fr_0.8fr]">
      <div className="relative min-h-[420px] overflow-hidden rounded-[28px] glass">
        ...
        <div className="relative flex h-full flex-col justify-end p-8">
          ...
          <h1 className="mt-2 max-w-xl text-4xl font-semibold leading-tight md:text-5xl">{t.heroTitle}</h1>
          <p className="mt-3 max-w-lg text-[var(--muted)]">{t.heroLead}</p>
        </div>
      </div>
      <div className="grid gap-3">
        ...
        <Door to="/cockpit" code="ACFT" title={t.doorAc} hint={t.doorAcHint} meta={catalog ? `${catalog.aircraft.length} ${t.parts}` : '—'} level={t.open} />
...
function Door({
  ...
  return (
    <Link to={to} className="glass group rounded-3xl p-5 transition hover:-translate-y-0.5">
